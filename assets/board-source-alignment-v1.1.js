(() => {
  ...
  // A single verified roster feeds the Board and reusable profile template.
  pageConfigs['board-of-directors'].cards=window.NEVSheBoardFacts.map(p=>[p.name,p.role,p.job?p.job+', '+p.org:'NEVShe Board of Directors',p.image?'shared:client-supplied/'+p.image:null]);
  ...
  const personCards = (cards) => `<div class="board-person-grid">${cards.map((x) => `<article class="board-person-card">${x[3] ? `<img class="board-person-photo" src="${img(x[3])}" alt="Portrait of ${x[0]}">` : `<div class="board-missing-portrait" role="img" aria-label="Portrait not yet supplied">${portraitPlaceholder}</div>`}<div class="board-person-copy"><div class="meta">${x[1]}</div><h3>${x[0]}</h3><p>${x[2]}</p><a class="board-profile-link" href="?profile=${profileSlug(x[0])}#profile">View profile ${icon('arrow')}</a></div></article>`).join('')}</div>`;
  ...
  function renderBoardProfile(c, person) {
    const fact=window.NEVSheBoardFacts.find(p=>p.slug===profileSlug(person[0]));
    const note=fact.bioState==='staging'?'The supplied full biography is reserved for private staging.':'A full biography has not been supplied.';
    return `<section class="section board-profile-view" id="profile"><div class="container"><a class="board-profile-back" href="./">${icon('arrow')} Back to all board members</a><div class="board-profile-layout"><div class="board-profile-portrait">${person[3]?`<img src="${img(person[3])}" alt="Portrait of ${person[0]}">`:`<div class="board-missing-portrait" role="img" aria-label="Portrait not yet supplied">${portraitPlaceholder}</div>`}</div><article class="board-profile-copy"><div class="eyebrow" style="color:var(--violet)">Individual board profile</div><div class="meta">${person[1]}</div><h1>${person[0]}</h1><p class="board-profile-role">${person[2]}</p><div class="notice">${note}</div><a class="btn btn-secondary" href="./">Return to board directory ${icon('arrow')}</a></article></div></div></section>`;
  }
  ...
  function renderHub() {
    ...
    const motionApproved = new Set(['home', 'events', 'education', 'membership', 'sponsorship', 'sponsor-directory', 'contact']);
