(() => {
  ...
  const currentVersions = {
    'home': 'v1.6',
    'about-nevshe': 'v1.5',
    'board-of-directors': 'v1.5',
    'events': 'v1.4',
    'education': 'v1.4',
    'membership': 'v1.4',
    'member-directory': 'v1.5',
    'sponsorship': 'v1.7',
    'sponsor-directory': 'v1.6',
    'resources': 'v1.5',
    'news': 'v1.6',
    'contact': 'v1.5',
    'privacy-policy': 'v1.3',
  ...
  const pageConfigs = {
    'home': {
      ...
      storyText: 'Healthcare facilities depend on energy, water, air, safety, knowledge, and people working together every day.',
      ...
      sections: [
        ['Built for the people behind healthcare', 'NEVShe connects facility operators, engineers, technicians, healthcare leaders, and industry partners across Nevada in one practical, welcoming, statewide community.'],
    ...
    'about-nevshe': {
      ...
      intro: 'Learn about NEVShe’s role, who it serves, and how its ASHE chapter connection supports education and collaboration across Nevada.',
    ...
    'events': {
      ...
      cards: [['Featured upcoming event','Date and venue pending','Program details will appear here when announced.'],['Education session','Details coming soon','Find speakers, objectives, CE/CEC information, an agenda, and registration.'],['Statewide networking','Details coming soon','In-person, virtual, and hybrid event opportunities will appear here.']]
    },
    'education': {
      ...
      sections: [['Education with operational relevance','Learning centers on the real systems, standards, and leadership challenges facilities teams navigate every day.'],['Learning in multiple formats','Explore upcoming sessions, webinars, presentations, certification links, topic submissions, and future on-demand material.']],
    ...
    'membership': {
      ...
      cards: [['Learn','Connect with education and resources relevant to healthcare facilities.','Membership benefit'],['Connect','Build relationships with operators, leaders, and industry partners statewide.','Membership benefit'],['Contribute','Share experience and help strengthen Nevada’s healthcare facilities community.','Membership benefit']],
      faq: [['Who should join?','Membership eligibility and membership types will be listed here when confirmed.'],['How do I register?','Select Join NEVShe to open the current registration form and continue to payment.'],['How is membership processed?','Registration is reviewed and processed by NEVShe.']]
    },
    'member-directory': {
    ...
    'sponsorship': {
      ...
      cards: [['Chapter visibility','Recognition across sponsor areas and chapter communications.','Benefits pending'],['Community support','Align with education, collaboration, and professional development in Nevada.','Benefits pending'],['Lasting connection','Build relationships with healthcare facilities professionals and leaders.','Benefits pending']],
      faq: [['What sponsorship levels are available?','Names, pricing, benefits, and availability will be listed here when confirmed.'],['Can we sponsor an event?','Opportunities may include annual chapter support and event-specific sponsorship when available.'],['How do we register?','Select Become a Sponsor to open the current registration form.']]
    },
    'sponsor-directory': {
    ...
    'news': {
      ...
      cards: [['Featured story title pending','Chapter News','A concise story excerpt will appear here when the article is available.'],['Article title pending','Industry Updates','News and updates will appear here as they are published.'],['Article title pending','Member Spotlight','Member stories will include a title, image, category, and short introduction.']]
    },
    'contact': {
  ...
  const featureCards = (cards, directory = false) => `<div class="grid grid-3">${cards.map((x,i) => directory ? `<article class="card directory-card placeholder"><div class="avatar">${String(i+1).padStart(2,'0')}</div><div><div class="meta">${x[1]}</div><h3>${x[0]}</h3><p>${x[2]}</p></div></article>` : `<article class="card"><div class="card-icon">${['✦','↗','◇','✓','◌','＋'][i%6]}</div><div class="meta">${x[1]}</div><h3>${x[0]}</h3><p>${x[2]}</p></article>`).join('')}</div>`;
  const linkedCards = (cards) => `<div class="grid grid-3">${cards.map((x,i) => `<article class="card"><div class="card-icon">${['✦','↗','◇'][i%3]}</div><h3>${x[0]}</h3><p>${x[1]}</p><span class="card-link">${x[2]} →</span></article>`).join('')}</div>`;
  ...
  function renderHome(c) {
    return `${pageHero(c,true)}
      ...
      <section class="section section-dark"><div class="container"><div class="split"><div>${sectionHeading('Featured event','A clear path from interest to registration.','Find upcoming dates, locations, speakers, learning objectives, CE/CEC details, agenda files, and registration information in one place.') }<div class="hero-actions"><a class="btn btn-primary" href="${local('events')}">View upcoming events ${icon('arrow')}</a></div></div><article class="card"><div class="meta">Featured event • Details coming soon</div><h3>Next NEVShe learning opportunity</h3><p>Date, format, location, registration details, and post-event resources will appear here.</p><span class="card-link">Event information coming soon →</span></article></div></div></section>
      <section class="section"><div class="container"><div class="quote"><blockquote>“One statewide community, connected by the work behind every safe and resilient healthcare environment.”</blockquote></div></div></section>
  ...
  function renderStandard(c) {
    return `${pageHero(c)}<section class="section"><div class="container"><div class="grid grid-3">${c.sections.map((s,i)=>`<article class="card"><div class="card-icon">${i+1}</div><h3>${s[0]}</h3><p>${s[1]}</p></article>`).join('')}</div></div></section>
      ...
      <section class="section"><div class="container"><div class="cta-band"><div class="eyebrow">Next step</div><h2>${c.cta ? c.cta[0] : 'Continue exploring'}</h2><p>Connect with NEVShe and find the next opportunity to learn, participate, or ask a question.</p><div class="hero-actions"><a class="btn btn-secondary" href="${c.cta ? c.cta[1] : local('contact')}">${c.cta ? c.cta[0] : 'Contact NEVShe'} ${icon('arrow')}</a></div></div></div></section>`;
  }
  ...
  function renderDirectory(c) {
    return `${pageHero(c)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search directory" placeholder="Search this directory"><button class="btn btn-primary" type="button">Search</button></div>${c.filters ? `<div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}</div></section>
      <section class="section section-alt"><div class="container">${sectionHeading('Directory','Browse organizations and chapter leadership.','Confirmed names, organizations, roles, and public profile details will appear here.')}${featureCards(c.cards,true)}</div></section>
      ${c.supportImage ? visualStory(c) : ''}
      <section class="section"><div class="container"><div class="notice"><strong>Privacy:</strong> the Member Directory includes organization-level information only and excludes personal email addresses, direct phone numbers, accounts, and private profiles.</div></div></section>`;
  }
  ...
  function renderArchive(c) {
    return `${pageHero(c)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search ${c.name}" placeholder="Search ${c.name.toLowerCase()}"><button class="btn btn-primary" type="button">Search</button></div><div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div></div></section>
      <section class="section section-alt" id="library"><div class="container">${sectionHeading(`Browse ${c.name.toLowerCase()}`,'Find the information that matters to you.','Use search and filters to narrow the collection. New items will appear here as they are published.')}${featureCards(c.cards)}</div></section>
  ...
  function renderHub() {
    ...
    const currentTemplateVersions = {'global-header-desktop-navigation':'v1.2','full-screen-mobile-navigation':'v1.2','global-footer':'v1.3','interior-page-hero':'v1.2','event-archive-card-single':'v1.2','sponsor-directory-card-marquee':'v1.2','resource-archive-card-single':'v1.2','board-directory-card':'v1.2','member-organization-directory-card':'v1.2','news-archive-card-single-post':'v1.2','search-no-results-404':'v1.2','contact-form-notification-routing':'v1.2','global-component-library':'v1.3'};
