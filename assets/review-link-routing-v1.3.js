/* Routing/metadata only: no content, style, approval or motion changes. */
(() => {
  'use strict';
  ...
  if(kind==='hub'){
    ...
    if(summary)summary.innerHTML='<span><strong>4</strong> source / required-field corrections · review</span><span><strong>26</strong> link-only refreshes · prior review states retained</span>';
    const intro=document.querySelector('.hub-hero .container > p');
    if(intro)intro.textContent='Required Contact fields and source-backed Board information are ready for review. All review links are current; unrelated design and motion are unchanged.';
    const spot=document.querySelector('.detail-spotlight .section-heading p');
    if(spot)spot.textContent='T6 v1.8 and T13 v1.9 are internally approved, waiting client approval. Later shell revisions retain their separate review state. Board/profile information now matches supplied client sources.';
  }
})();
