(() => {
  ...
  const aboutEditorialFeature = (c) => {
    ...
    return `<section class="section section-alt about-group-story"><div class="container">${sectionHeading(content.eyebrow, content.title, content.intro)}<figure class="about-group-photo"><img src="${editorialImage}" alt="Four people standing together in front of an ASHE event backdrop"><figcaption>Client-supplied chapter photograph. Names and final publication caption remain pending client confirmation.</figcaption></figure><div class="about-group-copy-grid">${content.sections.map(([title, text], index) => `<article><div class="editorial-kicker">${labels[index]}</div><h3>${title}</h3><p>${text}</p></article>`).join('')}</div></div></section>`;
  };
  ...
  function renderHub() {
    ...
    const motionApproved = new Set(['home', 'events', 'education', 'membership', 'sponsorship', 'contact']);
