(() => {
  ...
  const currentVersions = {
    'home': 'v1.12',
    'about-nevshe': 'v1.12',
    'board-of-directors': 'v1.11',
    'events': 'v1.8',
    'education': 'v1.11',
    'membership': 'v1.9',
    'member-directory': 'v1.11',
    'sponsorship': 'v1.12',
    'sponsor-directory': 'v1.11',
    'resources': 'v1.10',
    'news': 'v1.11',
    'contact': 'v1.10',
    'privacy-policy': 'v1.3',
  ...
  const pageConfigs = {
    ...
    'board-of-directors': {
      ...
      cards: [
        ...
        ['Brian Foote','Board Member','Serving NEVShe’s statewide professional community.','shared:client-supplied/brian-foote.jpeg'],
        ['Hans Gottschaldt','Board Member','Serving NEVShe’s statewide professional community.','shared:client-supplied/hans-gottschaldt.jpeg']
      ]
    },
    'events': {
    ...
    'member-directory': {
      ...
      cards: [['Member organization','Healthcare provider','Supporting Nevada’s healthcare facilities community.'],['Member organization','Engineering','Supporting Nevada’s healthcare facilities community.'],['Member organization','Construction','Supporting Nevada’s healthcare facilities community.'],['Member organization','Facilities services','Supporting Nevada’s healthcare facilities community.'],['Member organization','Technology','Supporting Nevada’s healthcare facilities community.'],['Member organization','Industry partner','Supporting Nevada’s healthcare facilities community.']]
    },
    'sponsorship': {
    ...
    'news': {
      ...
      archiveHeading: ['Latest NEVShe news','Chapter updates, professional perspectives, and stories from the community.','Published articles will appear here with a date, category, image, and short introduction. Use the categories to narrow the collection as it grows.'],
  ...
  const personCards = (cards) => `<div class="board-person-grid">${cards.map((x) => `<article class="board-person-card">${x[3] ? `<img class="board-person-photo" src="${img(x[3])}" alt="Portrait of ${x[0]}">` : `<div class="avatar person-avatar" aria-label="Portrait placeholder">${portraitPlaceholder}</div>`}<div class="board-person-copy"><div class="meta">${x[1]}</div><h3>${x[0]}</h3><p>${x[2]}</p></div></article>`).join('')}</div>`;
  ...
  function renderMemberDirectory(c) {
    return `${pageHero(c)}
      <section class="section member-directory-browser"><div class="container">${sectionHeading('Member organizations','Find an organization connected to NEVShe.','Search by organization name or browse by industry. Each listing provides a concise public description and a direct path to the organization’s website.')}<div class="directory-search-row"><div class="search-panel"><input type="search" aria-label="Search member organizations" placeholder="Search member organizations"><button class="btn btn-primary" type="button">Search</button></div><div class="directory-count"><strong>Member directory</strong><span>Organization listings</span></div></div><div class="filter-bar">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>${organizationTiles(c.cards)}</div></section>
      ${c.supportImage ? visualStory(c) : ''}
      <section class="section section-alt"><div class="container"><div class="directory-privacy"><div class="eyebrow" style="color:var(--violet)">Member privacy</div><h2>Public organization information only.</h2><p>The directory includes organization names, approved logos, industry categories, public descriptions, and website links. Personal member contact details and private profiles are not published.</p></div></div></section>`;
  }

  function renderResources(c) {
    const topics = [
      ['Codes & regulatory guidance','Requirements and references','Find approved links and documents related to codes, standards, accreditation, and regulatory information.'],
      ['Emergency preparedness','Planning and response','Browse resources that support continuity planning, readiness, response, and resilient healthcare environments.'],
      ['Water management','Safety, quality, and stewardship','Explore practical guidance for water-management programs, facility risk, quality, and responsible use.'],
      ['Sustainability & energy','Efficiency and environmental performance','Find information that supports energy awareness, efficiency, sustainability, and responsible facility operations.'],
      ['Certification & education','Professional development','Connect with approved certification information, learning references, and continuing-education material.'],
      ['NEVShe materials','Chapter presentations and files','Return to approved presentations, event resources, chapter documents, and other shared material.']
    ];
    return `${pageHero(c)}
      <section class="section resource-intro-section"><div class="container"><div class="resource-intro"><div class="resource-intro-media" role="img" aria-label="Healthcare facilities professionals reviewing technical information" style="--resource-image:url('${img(c.supportImage)}')"></div><div class="resource-intro-copy"><div class="eyebrow" style="color:var(--violet)">A practical professional library</div><h2>Start with the question in front of you.</h2><p>Healthcare facility work spans codes, safety, emergency readiness, water, energy, operations, and professional development. The NEVShe Resource Library brings those subjects into one clear place so visitors can find useful material without digging through the rest of the site.</p><p>Every published item will identify its source, topic, format, and publication information before it opens.</p></div></div></div></section>
      <section class="section section-alt resource-browser-section" id="library"><div class="container">${sectionHeading('Resource library','Search by topic, source, or format.','Use the search field or select a subject below. The library can grow without turning into one long, hard-to-scan list.')}<div class="resource-search-shell"><div class="search-panel"><input type="search" aria-label="Search resources" placeholder="Search resources"><button class="btn btn-primary" type="button">Search</button></div><div class="filter-bar">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div></div><div class="resource-topic-grid">${topics.map(([title,meta,text])=>`<article class="resource-topic"><div class="meta">${meta}</div><h3>${title}</h3><p>${text}</p><span class="card-link">Browse this topic →</span></article>`).join('')}</div></div></section>
      <section class="section"><div class="container"><div class="resource-assurance"><div><div class="eyebrow" style="color:var(--paper)">Clear before you click</div><h2>Know the source and format first.</h2></div><p>Document, external website, presentation, recording, and chapter-file links will be labeled clearly so professionals can choose the resource that fits their need.</p></div></div></section>`;
  }

  function renderNews(c) {
    const newsTypes = [
      ['Chapter news','Announcements, leadership updates, milestones, and chapter activity.'],
      ['Industry updates','Relevant developments and professional context for healthcare facilities.'],
      ['Event recaps','Approved highlights, photographs, presentations, and resources from NEVShe programs.']
    ];
    return `${pageHero(c)}${editorialContent('news', true)}${c.supportImage ? visualStory(c) : ''}
      <section class="section section-alt" id="library"><div class="container">${sectionHeading(...c.archiveHeading)}<div class="filter-bar news-filter-bar">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div><div class="news-coming-grid">${newsTypes.map(([title,text])=>`<article class="news-coming-card"><div class="meta">Articles coming soon</div><h3>${title}</h3><p>${text}</p></article>`).join('')}</div></div></section>`;
  }
  ...
  function renderHub() {
    ...
    const motionApproved = new Set(['home', 'events', 'membership']);
    ...
    const pageCards = pages.map(([slug,c])=>{ const v=currentVersions[slug]; const approved=motionApproved.has(slug); return `<a class="review-card${approved ? ' motion-approved' : ''}" href="pages/${slug}-${v}/" ${openNew}><span class="review-card-top"><span class="number">${String(c.n).padStart(2,'0')}</span><span class="review-status ${approved ? 'approved' : 'review'}">${approved ? 'Approved for motion' : 'In review'}</span></span><h3>${c.name}</h3><p>${c.intro}</p><footer><span>Page mockup</span><span>${v} ↗</span></footer></a>`; }).join('');
