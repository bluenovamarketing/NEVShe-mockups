(() => {
  ...
  function renderDirectory(c) {
    ...
    return `${pageHero(c)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search directory" placeholder="Search this directory"><button class="btn btn-primary" type="button">Search</button></div>${c.filters ? `<div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}</div></section>
      <section class="section section-alt"><div class="container">${sectionHeading('Directory','Browse organizations and chapter leadership.','Confirmed names, organizations, roles, and public profile details will appear here.')}${featureCards(c.cards,id === 'board-of-directors' ? 'people' : true)}</div></section>
      ${c.supportImage ? visualStory(c) : ''}
      ${privacyNote}`;
  }
  ...
  function renderHub() {
    ...
    const currentTemplateVersions = {'global-header-desktop-navigation':'v1.2','full-screen-mobile-navigation':'v1.2','global-footer':'v1.3','interior-page-hero':'v1.2','event-archive-card-single':'v1.2','sponsor-directory-card-marquee':'v1.2','resource-archive-card-single':'v1.2','board-directory-card':'v1.3','member-organization-directory-card':'v1.2','news-archive-card-single-post':'v1.2','search-no-results-404':'v1.2','contact-form-notification-routing':'v1.2','global-component-library':'v1.3'};
