(() => {
  ...
  const currentVersions = {
    'home': 'v1.10',
    'about-nevshe': 'v1.9',
    'board-of-directors': 'v1.9',
    'events': 'v1.7',
    'education': 'v1.8',
    'membership': 'v1.7',
    'member-directory': 'v1.9',
    'sponsorship': 'v1.10',
    'sponsor-directory': 'v1.9',
    'resources': 'v1.8',
    'news': 'v1.9',
    'contact': 'v1.8',
    'privacy-policy': 'v1.3',
  ...
  const editorialContent = (slug, alt = false) => {
    ...
    if (!content) return '';
    return `<section class="section editorial-content-section ${alt ? 'section-alt' : ''}"><div class="container">${sectionHeading(content.eyebrow, content.title, content.intro)}<div class="editorial-story-grid">${content.sections.map(([title, text]) => `<article class="editorial-story"><span class="editorial-story-marker" aria-hidden="true"></span><div><h3>${title}</h3><p>${text}</p></div></article>`).join('')}</div></div></section>`;
  };
  ...
  function renderDirectory(c) {
    ...
    return `${pageHero(c)}${editorialContent(id, true)}${cardSection(c.overviewHeading,c.overview)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search directory" placeholder="Search this directory"><button class="btn btn-primary" type="button">Search</button></div>${c.filters ? `<div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}</div></section>
  ...
  function renderMemberDirectory(c) {
    return `${pageHero(c)}${editorialContent('member-directory', true)}${cardSection(c.overviewHeading,c.overview)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search member organizations" placeholder="Search member organizations"><button class="btn btn-primary" type="button">Search</button></div>${c.filters ? `<div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}</div></section>
  ...
  function renderArchive(c) {
    return `${pageHero(c)}${editorialContent(id, true)}${cardSection(c.overviewHeading,c.overview)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search ${c.name}" placeholder="Search ${c.name.toLowerCase()}"><button class="btn btn-primary" type="button">Search</button></div><div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div></div></section>
  ...
  function renderConversion(c) {
    return `${pageHero(c)}${editorialContent(id, true)}<section class="section"><div class="container">${sectionHeading('Why participate','Learn, connect, and support the statewide community.','NEVShe participation is centered on professional education, statewide relationships, and stronger healthcare environments. Final membership or sponsorship terms will be published after chapter confirmation.')}${featureCards(c.cards)}</div></section>
      ${c.supportImage ? visualStory(c) : ''}
