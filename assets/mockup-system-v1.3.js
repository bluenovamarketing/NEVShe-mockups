(() => {
  ...
  const pageConfigs = {
    'home': {
      n: 1, name: 'Home', type: 'home', image: 'statewide-healthcare-hero-v1.png', motion: true,
      supportImage: 'v2:home-connected-campus-v2.png', storyMotion: true,
    ...
    'about-nevshe': {
      n: 2, name: 'About NEVShe', type: 'standard', image: 'v2:about-statewide-network-v2.png', motion: true,
      supportImage: 'v2:about-statewide-campus-v2.png', storyMotion: true,
      storyEyebrow: 'Statewide by design', storyTitle: 'One chapter across a wide, complex state.',
      storyText: 'An animated network layer carries the statewide idea beyond the hero—connecting geography, facilities, disciplines, and professional relationships without inventing chapter-history details.',
    ...
    'sponsorship': {
      ...
      supportImage: 'v2:sponsorship-workshop-v2.png',
      storyEyebrow: 'Support made visible', storyTitle: 'Partnership becomes useful participation.',
      storyText: 'The supporting image shifts the story from abstract recognition to a tangible learning experience, while exact sponsorship levels, benefits, and financial terms remain pending.',
    ...
    'sponsor-directory': {
      n: 9, name: 'Sponsor Directory', type: 'directory', image: 'v2:sponsor-directory-showcase-v2.png',
    ...
    'news': {
      n: 11, name: 'News', type: 'archive', image: 'v2:news-infrastructure-story-v2.png', eyebrow: 'Chapter and industry updates',
  ...
  const reviewBar = (label) => `<div class="review-bar"><a href="${root}">← Complete mockup hub</a><div class="review-meta"><span class="review-pill">${label}</span>${version ? `<span class="review-pill">${version}</span>` : ''}<span class="review-pill">Agency draft • 2026-08-30</span></div></div>`;
  ...
  const motionField = (variant = 'story') => `<div class="motion-field motion-field-${variant}" aria-hidden="true"><span class="motion-orbit motion-orbit-a"></span><span class="motion-orbit motion-orbit-b"></span><span class="motion-node motion-node-a"></span><span class="motion-node motion-node-b"></span><span class="motion-node motion-node-c"></span><span class="motion-beam motion-beam-a"></span><span class="motion-beam motion-beam-b"></span></div>`;
  ...
  const pageHero = (c, home = false) => {
    ...
    if (home) return `<section class="hero"${style}><div class="hero-lines"></div>${c.motion ? motionField('hero') : ''}<div class="hero-content"><div class="hero-copy"><span class="draft-chip">Agency draft</span><div class="eyebrow" style="margin-top:22px">${c.eyebrow}</div><h1>${c.title}</h1><p>${c.intro}</p><div class="hero-actions">${actions}</div></div></div></section>`;
    return `<section class="interior-hero ${c.image ? 'has-image' : ''}"${style}>${c.motion ? motionField('interior') : ''}<div class="interior-hero-inner"><div class="breadcrumbs"><a href="${local('home')}">Home</a><span>/</span><span>${c.name}</span></div><div><span class="draft-chip">Agency draft</span><div class="eyebrow" style="margin-top:20px">${c.eyebrow}</div><h1>${c.title}</h1><p>${c.intro}</p>${actions ? `<div class="hero-actions">${actions}</div>` : ''}</div></div></section>`;
  };
  ...
  const visualStory = (c) => `<section class="section visual-story-section"><div class="container"><div class="visual-story ${c.storyMotion ? 'is-animated' : ''}"><div class="visual-story-media" style="--story-image:url('${img(c.supportImage)}')">${c.storyMotion ? motionField('story') : '<span class="visual-sweep" aria-hidden="true"></span>'}<span class="visual-story-label">Unique page image</span></div><div class="visual-story-copy"><div class="eyebrow" style="color:var(--violet)">${c.storyEyebrow}</div><h2>${c.storyTitle}</h2><p>${c.storyText}</p><div class="visual-facts" aria-label="Visual direction notes"><span>Agency draft</span><span>Page-specific asset</span><span>${c.storyMotion ? 'Motion concept' : 'Editorial break'}</span></div></div></div></div></section>`;
