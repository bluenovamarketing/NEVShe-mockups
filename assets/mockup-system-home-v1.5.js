(() => {
  ...
  const homeCardRoutes = {Education:'education', Events:'events', Membership:'membership'};
  const featureCards = (cards, directory = false) => `<div class="grid ${cards.length > 4 ? 'grid-3' : 'grid-3'}">${cards.map((x,i) => directory ? `<article class="card directory-card placeholder"><div class="avatar">${String(i+1).padStart(2,'0')}</div><div><div class="meta">${x[1]}</div><h3>${x[0]}</h3><p>${x[2]}</p></div></article>` : `<article class="card"><div class="card-icon">${['✦','↗','◇','✓','◌','＋'][i%6]}</div><h3>${x[0]}</h3><p>${x[1]}</p><a class="card-link" href="${homeCardRoutes[x[0]] ? local(homeCardRoutes[x[0]]) : '#'}">${x[2]} →</a></article>`).join('')}</div>`;
