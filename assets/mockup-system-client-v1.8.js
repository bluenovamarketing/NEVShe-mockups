(() => {
  ...
  const currentVersions = {
    'home': 'v1.8',
    'about-nevshe': 'v1.7',
    'board-of-directors': 'v1.7',
    'events': 'v1.5',
    'education': 'v1.6',
    'membership': 'v1.5',
    'member-directory': 'v1.7',
    'sponsorship': 'v1.8',
    'sponsor-directory': 'v1.7',
    'resources': 'v1.6',
    'news': 'v1.7',
    'contact': 'v1.6',
    'privacy-policy': 'v1.3',
  ...
  function renderDirectory(c) {
    ...
    return `${pageHero(c)}${cardSection(c.overviewHeading,c.overview)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search directory" placeholder="Search this directory"><button class="btn btn-primary" type="button">Search</button></div>${c.filters ? `<div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}</div></section>
  ...
  function renderMemberDirectory(c) {
    return `${pageHero(c)}${cardSection(c.overviewHeading,c.overview)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search member organizations" placeholder="Search member organizations"><button class="btn btn-primary" type="button">Search</button></div>${c.filters ? `<div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}</div></section>
  ...
  function renderArchive(c) {
    return `${pageHero(c)}${cardSection(c.overviewHeading,c.overview)}<section class="section"><div class="container"><div class="search-panel"><input type="search" aria-label="Search ${c.name}" placeholder="Search ${c.name.toLowerCase()}"><button class="btn btn-primary" type="button">Search</button></div><div class="filter-bar" style="margin-top:18px">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div></div></section>
