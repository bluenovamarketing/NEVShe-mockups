(() => {
  ...
  const currentVersions = {
    ...
    'education': 'v1.11',
    'membership': 'v1.9',
    'member-directory': 'v1.12',
    'sponsorship': 'v1.12',
    'sponsor-directory': 'v1.12',
    'resources': 'v1.11',
    'news': 'v1.12',
    'contact': 'v1.10',
    'privacy-policy': 'v1.4',
  ...
  const pageConfigs = {
    ...
    'member-directory': {
      ...
      intro: 'The directory pairs each member organization with an approved public representative, role, logo, and website while keeping personal contact details private.',
  ...
  function renderMemberDirectory(c) {
    return `${pageHero(c)}
      <section class="section member-directory-browser"><div class="container">${sectionHeading('Member organizations','Find an organization and its NEVShe representative.','Search by organization name. Each listing pairs the organization with an approved public representative and professional role, without publishing personal contact details.')}<div class="directory-search-row"><div class="search-panel"><input type="search" aria-label="Search member organizations" placeholder="Search organizations or representatives"><button class="btn btn-primary" type="button">Search</button></div><div class="directory-count"><strong>Member directory</strong><span>Organization + representative</span></div></div>${c.filters.length > 1 ? `<div class="filter-bar">${c.filters.map((f,i)=>`<button class="filter" type="button" aria-pressed="${i===0}">${f}</button>`).join('')}</div>` : ''}${organizationTiles(c.cards)}</div></section>
      ${c.supportImage ? visualStory(c) : ''}
      <section class="section section-alt"><div class="container"><div class="directory-privacy"><div class="eyebrow" style="color:var(--violet)">Member privacy</div><h2>Useful context without private contact information.</h2><p>The directory may show an approved representative’s name and professional role alongside the organization. Personal email addresses, direct phone numbers, private profiles, and member accounts are not published.</p></div></div></section>`;
  }

  function renderSponsorDirectory(c) {
    return `${pageHero(c)}
      <section class="section sponsor-directory-simple"><div class="container">${sectionHeading('Sponsor directory','Organizations that support NEVShe.','Browse confirmed sponsor organizations by name. Every listing is selected, approved, and published by NEVShe.')}<div class="search-panel"><input type="search" aria-label="Search sponsor organizations" placeholder="Search sponsor organizations"><button class="btn btn-primary" type="button">Search</button></div><div class="sponsor-directory-grid">${organizationTiles(c.cards)}</div></div></section>
      ${c.supportImage ? visualStory(c) : ''}`;
  }
  ...
  function renderResources(c) {
    return `${pageHero(c)}
      ...
      <section class="section section-alt resource-browser-section" id="library"><div class="container">${sectionHeading(...c.archiveHeading)}<div class="resource-search-shell"><div class="search-panel"><input type="search" aria-label="Search resources" placeholder="Search published resources"><button class="btn btn-primary" type="button">Search</button></div></div><div class="resource-empty-state"><div><div class="eyebrow" style="color:var(--violet)">Library status</div><h2>No resources have been published yet.</h2><p>Approved presentations, documents, recordings, chapter files, and trusted links will appear here as NEVShe adds them.</p></div><a class="btn btn-secondary" href="${local('education')}">Explore education ${icon('arrow')}</a></div></div></section>
  ...
  function renderNews(c) {
    return `${pageHero(c)}${c.supportImage ? visualStory(c) : ''}
      <section class="section section-alt" id="library"><div class="container"><div class="news-empty-state"><div>${sectionHeading(...c.archiveHeading)}<h3>No articles have been published yet.</h3><p>The first confirmed NEVShe announcements and stories will appear here after publication.</p></div><div class="news-empty-action"><a class="btn btn-primary" href="${notifyUrl}">Get notifications ${icon('arrow')}</a></div></div></div></section>`;
  }
  ...
  function renderHub() {
    ...
    const motionApproved = new Set(['home', 'events', 'education', 'membership', 'sponsorship', 'contact', 'search-results', '404']);
