(() => {
  ...
  function renderHub() {
    ...
    const currentPageVersions = {'home':'v1.3','about-nevshe':'v1.3','board-of-directors':'v1.3','member-directory':'v1.3','sponsorship':'v1.3','sponsor-directory':'v1.3','news':'v1.3','contact':'v1.3'};
    const pageCards = pages.map(([slug,c])=>{ const v=currentPageVersions[slug] || 'v1.2'; return `<a class="review-card" href="pages/${slug}-${v}/" ${openNew}><span class="number">${String(c.n).padStart(2,'0')}</span><h3>${c.name}</h3><p>${c.intro}</p><footer><span>${v === 'v1.3' ? 'Image-led revision' : 'Page-only mockup'}</span><span>${v} ↗</span></footer></a>`; }).join('');
